...
/* Create test pattern as generated by OV7725 camera
   ...
   To save on memory we are using approximation of real test pattern
   generated by OV7725 camera.
...
#if ENABLE_TEST_PATTERN
    #define EXPECTED_VARIANCE 6531449l
    static int frame_count = 0;
    static int variance_mismatch_count = 0;

    unsigned long current_variance =  test_pattern_variance();
    frame_count++;
    if (current_variance != EXPECTED_VARIANCE) {
        variance_mismatch_count++;
    }
    printf("Frames / mismatch : %d / %d (%d%%), (%lu)\n", frame_count, variance_mismatch_count, 100 * variance_mismatch_count / frame_count, current_variance);
#endif
